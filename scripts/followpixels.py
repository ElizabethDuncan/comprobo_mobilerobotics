...
class Follow_Path:
	...
	def get_directed_angle(self):
		...
		print mini_goal, distance, self.current_ind, len(self.coords), self.done
		if distance < 0.1:
	...
	def main(self):
		...
		while not rospy.is_shutdown() and self.done == False:
			if self.directed_angle != None:
				if math.fabs(self.directed_angle) > 1:
					msg = Twist(angular=Vector3(z = self.directed_angle * 0.01))
				else:
					msg = Twist(linear=Vector3(x=0.2), angular=Vector3(z= self.directed_angle * 0.01))
				pub.publish(msg)
		if self.done == True:
			print "Done"
